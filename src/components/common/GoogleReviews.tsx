  ...
  useEffect(() => {
    async function fetchReviews() {
      try {
        ...
        // --- PROXY SWITCH ---
        // We are using CodeTabs here. It is often more robust for static sites.
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
          <div className="text-center md:text-left">
            <h2 className="text-4xl font-extrabold text-white mb-3 tracking-tight">What Our Clients Say</h2>
            <div className="flex items-center gap-3 justify-center md:justify-start">
              ...
              <span className="text-teal-100 font-medium text-sm">Based on {reviews.length || "Google"} Reviews</span>
            </div>
          </div>
          <a href={`https://search.google.com/local/writereview?placeid=${BUTTON_ID}`} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 bg-blue-950 hover:bg-blue-900 text-white font-semibold py-3 px-8 rounded-full transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 border border-blue-800">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {!loading ? (
                ...
                <div key={i} className="bg-white p-6 rounded-2xl shadow-xl border border-teal-800/20 flex flex-col h-full hover:shadow-2xl transition-shadow duration-300 transform hover:-translate-y-1">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 rounded-full bg-teal-50 border-2 border-teal-100 flex items-center justify-center overflow-hidden shrink-0">
                      {image ? <img src={image} alt={name} className="w-full h-full object-cover" /> : <span className="text-teal-700 font-bold text-lg">{name.charAt(0)}</span>}
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm leading-tight line-clamp-1">{name}</h4>
                      <span className="text-xs text-gray-500 block mt-0.5">{review.date || "Recently"}</span>
                    </div>
                  </div>
                  ...
                  <div className="relative flex-grow">
                    ...
                    <p className="text-gray-600 text-sm leading-relaxed line-clamp-4 relative z-0">{review.snippet || "No comment provided."}</p>
                  </div>
                  {review.link && (
            ...
            [1, 2, 3, 4].map((n) => (
              <div key={n} className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-teal-800 h-64 animate-pulse"></div>
            ))
          )}
        </div>
      </div>
